...
4. `npm test` — vitest over `test/` (24 suites: main-side parser/indexer/rename/customTitle/windowState/codexParser/codexThreadsDb/codexSessionIndex/codexRename/codexInputNotifications/bootCommand/agentEnv/submitDetect/ptyBinding/updater-core + shared messageCount + renderer-lib liveness/theme/navHistory/pins/clipboard/findMatches/messageGroups/codeLang). The renderer UI is not unit-tested. `ptyBinding` is the one suite that drives a stateful class (`PtyManager`) rather than a pure function — `node-pty` is stubbed via `vi.mock` and fake timers pin the submit ordering that new-Codex binding keys on.
