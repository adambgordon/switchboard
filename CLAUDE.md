...
4. `npm test` — vitest over `test/` (34 suites: main-side parser/indexer/rename/customTitle/windowState/windowFocus/codexParser/codexThreadsDb/codexSessionIndex/codexRename/codexInputNotifications/bootCommand/agentEnv/codexIdentity/ptyBinding/claudeParkedJobs/claudeJobs/updater-core + shared messageCount + renderer-lib liveness/focusSync/animationSync/theme/navHistory/pins/clipboard/findMatches/messageGroups/codeLang/rowIdentity/mdCopy/mathDelimiters/maxLiveScale + repo-hygiene lockfileRegistry). The renderer UI is not unit-tested. Two suites are unusual: `codexIdentity` drives the real `execFile` path against stub binaries (the branches deciding whether `lsof` output is trustworthy are what stand between a truncated probe and a *wrong* bind, so they can't be faked), and `ptyBinding` drives a stateful class (`PtyManager`) rather than a pure function, with `node-pty` stubbed via `vi.mock` and the identity resolver injected. Both were built by **mutation-checking**: revert a guard, confirm a named test fails. That found several tests that passed for the wrong reason — do it for any new guard here, or you can't tell a guard from a tautology. `mdCopy` was built the same way and is the strongest argument for the practice: mutation-checking its guards surfaced four real defects the tests had been passing straight over, including an unclamped `slice` that silently dropped a delimiter.
