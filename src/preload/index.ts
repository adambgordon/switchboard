...
const api: SwitchboardApi = {
  ...
  activateTabWorkspace: () => ipcRenderer.invoke(IPC.tabWorkspaceActivate),
  finishTabWorkspaceActivation: () => ipcRenderer.send(IPC.tabWorkspaceActivated),
  clearTabWorkspace: () => ipcRenderer.send(IPC.tabWorkspaceClear),
  onTabsElsewhere: (cb) => subscribe(IPC.tabsElsewhere, cb as never),
  revealConversation: (sessionId, mode) => ipcRenderer.send(IPC.conversationReveal, sessionId, mode),
  ...
  reportNavigation: (visit, record, revision) => ipcRenderer.send(IPC.navigationReport, visit, record, revision),
  stepNavigation: (direction) => ipcRenderer.send(IPC.navigationStep, direction),
  interruptNavigation: (revision) => ipcRenderer.send(IPC.navigationInterrupt, revision),
  completeNavigation: (requestId, visit) => ipcRenderer.send(IPC.navigationComplete, requestId, visit),
  onNavigationCancelled: (cb) => subscribe(IPC.navigationCancelled, cb as never),
  ...
  reserveBoundTab: (ptyId, oldSessionId, sessionId) => ipcRenderer.invoke(IPC.tabReserveBound, ptyId, oldSessionId, sessionId),
  commitBoundTab: (token) => ipcRenderer.send(IPC.tabCommitBound, token),
  cancelBoundTab: (token) => ipcRenderer.send(IPC.tabCancelBound, token),
