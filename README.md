...
- **Copy buttons** — hover a turn (beside its timestamp), a code block, a table, or a tool call/result for a copy button, and find another at the end of the transcript that exports the whole conversation as speaker-labeled Markdown with dividers. They honor the same **Copy as markdown** setting, and the turn button follows the same expanded-tool-run rule as ⌘C. The whole-conversation export always leaves tool I/O out, so it doesn't change with what you happen to have expanded. Each flashes a check when copied.
