...
- **Copy buttons** — hover a turn (beside its timestamp), a code block, a table, or a tool call/result for a copy button, and find another at the end of the transcript that exports the whole conversation as speaker-labeled Markdown with dividers. They honour the same **Copy as markdown** setting, and the turn button follows the same expanded-tool-run rule as ⌘C. The whole-conversation export always leaves tool I/O out, so it doesn't change with what you happen to have expanded. Each flashes a check when copied.
...
- **LaTeX math** renders in the Formatted view for both agents (`\(…\)` / `$…$` inline, `\[…\]` / `$$…$$` display). Detection is deliberately conservative: a message is treated as containing math only if it has a display equation on its own lines, and a bare `$…$` counts only in a message that also wrote display math with `$$`. That leaves `\[DEBUG\]`, `sed`/`jq` expressions, and `$PATH` alone — at the cost of occasionally showing real math as source, which is the intended trade.
