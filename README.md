...
- **Formatted ⇄ Terminal** — for a live conversation, click the view toggle or press **⌘J** in the focused pane to switch between the raw **Terminal** (where you type) and a **Formatted** view that renders both your prompts and the agent's replies as Markdown — with syntax-highlighted code blocks — and stays pinned to the latest message. The chosen view and each Formatted reading position stick per conversation while the app is open; hovering a link reveals its URL.
- **Copy from the transcript** — select any text in the **Formatted** view and ⌘C gives you the underlying Markdown, not the rendered text: headings keep their `#`, bold keeps its `**`, links keep their target. Select part of a styled run and you get the plain characters instead, never a stray half-delimiter. A code block copies bare unless your selection reaches past it, in which case the fence comes too; a selection spanning several messages arrives speaker-labeled. Expand a tool run and its calls and results travel with the copy; leave it collapsed and they don't. Turn it off in **Preferences → Application → Copy as markdown** to copy plain text instead.
- **Copy buttons** — hover a turn (beside its timestamp), a code block, a table, or a tool call/result for a copy button, and find another at the end of the transcript that exports the whole conversation as speaker-labeled Markdown with dividers. They honor the same **Copy as markdown** setting, and the turn button follows the same expanded-tool-run rule as ⌘C. Selecting exactly an inline code span copies the bare code — the URL or the error string, without its backticks — while a selection that runs past the span keeps them, so grabbing a value to use and quoting a sentence both do the obvious thing. **Right-click** an inline span for **Copy Code**, or a link for **Copy Link** / **Open Link in Browser**. The whole-conversation export always leaves tool I/O out, so it doesn't change with what you happen to have expanded. Each flashes a check when copied.
...
| `⌘J` | Toggle Formatted / Terminal in the focused pane (live conversations only) |
